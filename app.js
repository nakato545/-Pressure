...
function calcRiskScore(hpa, diff3h, diff6h, diff24h, humidity, precip, tempDrop6h) {
  ...
  // 24時間変化（5hPa=重症片頭痛の閾値, 10=一般者, 15=非敏感者）
  if      (a24 > 15) s += 4;
  else if (a24 > 10) s += 3;
  else if (a24 >  5) s += 2;
  else if (a24 >  2) s += 1;
...
function getLevelAtHour(pressures, h, humidity = [], precip = [], temps = []) {
  ...
  // 先読み補正（研究: 翌日の低気圧 gain=6.7）
  if      (Math.abs(diffFwd) > 4) score += 2;
  else if (Math.abs(diffFwd) > 2) score += 1;

  // 持続的低気圧補正（過去12h全て1010未満 → 追加リスク）
