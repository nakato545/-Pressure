...
function getLevel(hpa, diff3h, diff6h = 0, diff24h = 0) {
  const a3 = Math.abs(diff3h), a6 = Math.abs(diff6h), a24 = Math.abs(diff24h);
  if (hpa < 1000 || a3 > 6 || a24 > 12) return 'danger';
  if (hpa < 1005 || a3 > 4 || a6 > 7  || a24 > 8)  return 'alert';
  if (hpa < 1010 || a3 > 2 || a6 > 4  || a24 > 6)  return 'warn';
  return 'safe';
}
...
function getLevelAtHour(pressures, h) {
  ...
  const a3 = Math.abs(diff3h), a6 = Math.abs(diff6h), a24 = Math.abs(diff24h);
  if (cur < 1000 || a3 > 6 || a24 > 12) return 'danger';
  if (cur < 1005 || a3 > 4 || a6 > 7  || a24 > 8)  return 'alert';
  if (cur < 1010 || a3 > 2 || a6 > 4  || a24 > 6 || Math.abs(diffFwd) > 3) return 'warn';
  // 回復・急変局面：直近6hの最低値が1005未満かつ変化中
  const recentMin = Math.min(...pressures.slice(Math.max(0, h - 6), h + 1));
  if (recentMin < 1005 && Math.abs(diff3h) > 1) return 'warn';
  return 'safe';
}
