...
function calcRiskScore(hpa, diff1h, diff3h, diff6h, diff24h, humidity, precip, tempDrop6h) {
  ...
  if      (rateNow > 1.5)  s += 5; // 9 hPa/6h相当 — 台風級
  else if (rateNow > 1.0)  s += 4; // 6 hPa/6h相当 — 非常に急激
  else if (rateNow > 0.5)  s += 3; // 3 hPa/6h相当 — 急激 (12 hPa/24h)
  else if (rateNow > 0.33) s += 2; // 片頭痛発症ライン (8 hPa/24h)
  else if (rateNow > 0.21) s += 1; // 重症者の閾値 (5 hPa/24h)
  ...
  // 降水量
  if      (precip >= 2.0) s += 3;
  else if (precip >= 0.5) s += 2;
  else if (precip >= 0.1) s += 1;
  ...
  if (hpa < 1010 && humidity >= 75 && precip >= 0.2) s += 3;
