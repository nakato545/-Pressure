...
function updateDangerHoursLabel(startHour) {
  ...
  const html = periods.map(p => {
    ...
    const eh = (startHour + p.end + 1) % 24;
    return `<span style="color:${COLOR[p.worst]}">${sh}〜${eh}時（${LABEL[p.worst]}）</span>`;
  }).join('　');
...
function applyPressureData(data, currentHour, skipAI = false) {
  ...
  if (!skipAI) getDailyAdvice(displayLevel, cur, diff3, data.hourly.weather_code[currentHour], data.hourly.temperature_2m[currentHour]);
