...
// ── 危険度判定（論文ベース複合スコアリング）──
// 根拠: pubmed 36853848 (4375名・33万件) gain値による重み設定
// 最重要: 6h変化(gain=11.7) > 絶対気圧(10.1) > 湿度(7.1) > 24h変化 > 降雨 > 気温低下
function calcRiskScore(hpa, diff1h, diff3h, diff6h, diff24h, humidity, precip, tempDrop6h) {
  let s = 0;
  const a1  = Math.abs(diff1h);
  const a3  = Math.abs(diff3h);
  const a6  = Math.abs(diff6h);
  const a24 = Math.abs(diff24h);

  // ── 正規化変化率 (hPa/h) — 1h/3h/6hの最大値で「今の速さ」を評価 ──
  // 1h窓は÷1なので実レートそのまま。短時間急変（1-2h）の過小評価を防ぐ
  // 0.21 hPa/h = 5 hPa/24h（重症者閾値）, 0.33 = 8 hPa/24h（片頭痛発症ライン）
  const rateNow = Math.max(a1, a3 / 3, a6 / 6);
  if      (rateNow > 1.5)  s += 5; // 36 hPa/24h — 台風級
  else if (rateNow > 0.75) s += 4; // 18 hPa/24h — 非敏感者超過帯（研究: 15超えが非敏感者発症）
  else if (rateNow >= 0.5) s += 3; // 12 hPa/24h — 痛みほぼ確実（研究閾値「以上」）
  else if (rateNow > 0.33) s += 2; // 8 hPa/24h — 片頭痛発症ライン
  else if (rateNow > 0.21) s += 1; // 5 hPa/24h — 重症者の閾値

  // 絶対気圧（1003-1007 hPaが発作の50%集中、段差なく滑らかに）
  if      (hpa < 1003) s += 5;
  else if (hpa < 1007) s += 4;
  else if (hpa < 1009) s += 3;
  else if (hpa < 1011) s += 2;
  else if (hpa < 1013) s += 1;
  ...
  // 湿度（gain=7.1: 低気圧との複合で大幅リスク増、高気圧時は影響小）
  if (hpa < 1015) {
    if      (humidity >= 88) s += 4;
    else if (humidity >= 82) s += 3;
    else if (humidity >= 78) s += 2;
    else if (humidity >= 72) s += 1;
  }

  // 降水量（独立した因果関係の証拠は弱く、低気圧・湿度の代理指標として保守的に使用）
  if      (precip >= 2.0) s += 2;
  else if (precip >= 0.5) s += 1;

  // 気温急低下（低温+高湿度が最高強度と相関）
  if      (tempDrop6h > 5) s += 2;
  else if (tempDrop6h > 3) s += 1;

  // 複合ボーナス（低気圧+高湿度+雨 = 研究が示す最危険パターン）
  if (hpa < 1010 && humidity >= 75 && precip >= 0.5) s += 3;

  return s;
}

function scoreToLevel(s) {
  if (s >= 14) return 'danger';
  if (s >=  8) return 'alert';
  if (s >=  3) return 'warn';
  return 'safe';
}

function getLevelAtHour(pressures, h, humidity = [], precip = [], temps = []) {
  const cur   = pressures[h] ?? pressures[pressures.length - 1];
  const prev1 = pressures[Math.max(0, h - 1)]  ?? cur;
  const prev3 = pressures[Math.max(0, h - 3)]  ?? cur;
  const prev6 = pressures[Math.max(0, h - 6)]  ?? cur;
  const diff1h  = cur   - prev1;
  const diff3h  = cur   - prev3;
  const diff6h  = cur   - prev6;
  const diff24h = cur   - (pressures[Math.max(0, h - 24)] ?? cur);

  const curTemp    = temps[h]                   ?? 0;
  const prev6Temp  = temps[Math.max(0, h - 6)] ?? curTemp;
  const tempDrop6h = prev6Temp - curTemp;

  let score = calcRiskScore(
    cur, diff1h, diff3h, diff6h, diff24h,
    humidity[h] || 0, precip[h] || 0, tempDrop6h
  );

  // ── 加速度スコア（同方向に変化が加速中か）──
  const rateRecent = Math.abs(diff3h) / 3;
  const rateOlder  = Math.abs(prev3 - prev6) / 3;
  const sameDir    = Math.sign(diff3h) === Math.sign(prev3 - prev6);
  if (sameDir && rateRecent > rateOlder + 0.5 && rateRecent > 0.3) {
    score += rateRecent > rateOlder * 2 ? 2 : 1;
  }

  // ── 拡張先読みスコア（研究: 症状は6-24h前から始まる）──
  const len = pressures.length;
  const fwd3  = (pressures[Math.min(h + 3,  len - 1)] ?? cur) - cur;
  const fwd6  = (pressures[Math.min(h + 6,  len - 1)] ?? cur) - cur;
  const fwd12 = (pressures[Math.min(h + 12, len - 1)] ?? cur) - cur;
  const fwd1  = (pressures[Math.min(h + 1,  len - 1)] ?? cur) - cur;
  const fwdRate = Math.max(Math.abs(fwd1), Math.abs(fwd3) / 3, Math.abs(fwd6) / 6, Math.abs(fwd12) / 12);
  if      (fwdRate > 1.0)  score += 3;
  else if (fwdRate > 0.5)  score += 2;
  else if (fwdRate > 0.25) score += 1;

  // ── 持続的変化ボーナス（現在・近未来の両方でレートが高い = 24h累積大） ──
  const rateNow = Math.max(Math.abs(diff1h), Math.abs(diff3h) / 3, Math.abs(diff6h) / 6);
  if      (rateNow > 0.5  && fwdRate > 0.5)  score += 2;
  else if (rateNow > 0.33 && fwdRate > 0.33) score += 1;

  // ── 時間帯脆弱性補正（大気潮汐 + 概日リズムの複合効果）──
  // 4時は気圧日周最低点かつ生体的脆弱期のピーク（発作の40%が早朝集中）
  const hour = h % 24;
  if (hour >= 3 && hour < 8 && score >= 3) {
    score += 2;
  } else if (hour >= 14 && hour < 17 && score >= 3) {
    score += 1;
  }

  // ── 持続的低気圧補正（過去12h全て1010未満）──
  const recent = pressures.slice(Math.max(0, h - 12), h + 1);
  if (recent.length >= 6 && recent.every(p => p < 1010)) score += 2;
...
const CHEER_PATTERNS = {
  morning: [
    ...
    { icon: '🍳', emojis: ['🍳','🥚','🍞','☕','🍳','🥚','🍞','☕'], msg: '朝ごはん！' },
    { icon: '🌤', emojis: ['🌤','🌤','🌥','🌤','🌤','🌥','🌤','🌤'], msg: 'のんびりいこ！' },
    { icon: '💨', emojis: ['💨','🌿','💨','🍃','💨','🌿','💨','🍃'], msg: '深呼吸！' },
    { icon: '🫧', emojis: ['🫧','✨','🫧','💙','🫧','✨','🫧','💙'], msg: '無理せず～！' },
    { icon: '🌱', emojis: ['🌱','🌱','💚','🌱','🌱','💚','🌱','🌱'], msg: 'ゆっくりスタート！' },
    { icon: '🫶', emojis: ['💜','🌀','💜','🌀','💜','🌀','💜','🌀'], msg: '気を付けや\n気圧ケア' },
  ],
  afternoon: [
    { icon: '🌈', emojis: ['⭐','🌈','⭐','🌈','⭐','🌈','⭐','🌈'], msg: 'お昼だよ！' },
    { icon: '⚡', emojis: ['⚡','🔥','⚡','💪','⚡','🔥','⚡','💪'], msg: 'ひといき！' },
    { icon: '🍀', emojis: ['🍀','🍀','💚','🍀','🍀','💚','🍀','🍀'], msg: 'がんばれー！' },
    { icon: '💧', emojis: ['💧','💧','🌊','💧','💧','🌊','💧','💧'], msg: '水分補給だいじ！' },
    { icon: '😮‍💨', emojis: ['😮‍💨','💙','😮‍💨','✨','😮‍💨','💙','😮‍💨','✨'], msg: 'ちょっと休もう！' },
    { icon: '🧘', emojis: ['🧘','☁️','🧘','🌸','🧘','☁️','🧘','🌸'], msg: 'ひとやすみ～！' },
    { icon: '🌻', emojis: ['🌻','☀️','🌻','🌻','☀️','🌻','🌻','☀️'], msg: 'もう少し！' },
    { icon: '🎯', emojis: ['🎯','⚡','🎯','✨','🎯','⚡','🎯','✨'], msg: '今日も半分来たよ！' },
    { icon: '🫶', emojis: ['💜','🌀','💜','🌀','💜','🌀','💜','🌀'], msg: '気を付けや\n気圧ケア' },
  ],
  evening: [
    ...
    { icon: '🌆', emojis: ['🌆','🌆','✨','🌆','🌆','✨','🌆','🌆'], msg: '今日もお疲れ！' },
    { icon: '🍜', emojis: ['🍜','🍜','😋','🍜','🍜','😋','🍜','🍜'], msg: '晩めし！' },
    { icon: '🌸', emojis: ['🌸','💜','🌸','💜','🌸','💜','🌸','💜'], msg: 'ゆっくりしてね～！' },
    { icon: '💜', emojis: ['💜','✨','💜','✨','💜','✨','💜','✨'], msg: 'リラックス～！' },
    { icon: '📝', emojis: ['📝','😤','📝','💪','📝','😤','📝','💪'], msg: '課題(||ﾟДﾟ)' },
    { icon: '🫶', emojis: ['💜','🌀','💜','🌀','💜','🌀','💜','🌀'], msg: '気を付けや\n気圧ケア' },
  ],
  night: [
    ...
    { icon: '✨', emojis: ['✨','💫','✨','⭐','✨','💫','✨','💫'], msg: 'Zzz...' },
    { icon: '🌃', emojis: ['🌃','⭐','🌃','✨','🌃','⭐','🌃','✨'], msg: 'また明日～！' },
    { icon: '🫶', emojis: ['🫶','💜','🫶','💜','🫶','💜','🫶','💜'], msg: 'いい夢みてね！' },
    { icon: '🌛', emojis: ['🌛','⭐','🌛','💫','🌛','⭐','🌛','💫'], msg: 'はやく寝てね？' },
    { icon: '🕯', emojis: ['🕯','✨','🕯','🌙','🕯','✨','🕯','🌙'], msg: '脳を休めて～！' },
    { icon: '🧸', emojis: ['🧸','💤','🧸','🌙','🧸','💤','🧸','🌙'], msg: '体を休めて～！' },
    { icon: '🫶', emojis: ['💜','🌀','💜','🌀','💜','🌀','💜','🌀'], msg: '気を付けや\n気圧ケア' },
  ],
};
...
// ── ダジャレプール（随時追加可）──
const DAJARE_POOL = [
  { icon: '🥵', emojis: ['🥵','☀️','🥵','😅','🥵','☀️','🥵','😅'], msg: '猛暑はもうしょうがない' },
  { icon: '💸', emojis: ['💸','😱','💸','💳','💸','😱','💸','💳'], msg: '残高刻んだか～' },
  { icon: '🧨', emojis: ['🧨','💥','🧨','⬇️','🧨','💥','🧨','⬇️'], msg: 'クラッカーは良く落下～' },
  { icon: '🍋', emojis: ['🍋','🎣','🍋','🎣','🍋','🎣','🍋','🎣'], msg: 'ポン酢の一本釣り' },
];
...
function playCheer() {
  ...
  const useDajare = Math.random() < 1 / 10;
  const pat = useDajare
    ? DAJARE_POOL[Math.floor(Math.random() * DAJARE_POOL.length)]
  ...
  msgEl.style.cssText = 'font-size:17px;font-weight:600;color:var(--text);line-height:1.6;white-space:pre-line;text-align:center;';
...
async function fetchWeather(lat, lon) {
  const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&hourly=surface_pressure,temperature_2m,relative_humidity_2m,weather_code,wind_speed_10m,precipitation,precipitation_probability&daily=weather_code,temperature_2m_max,temperature_2m_min&timezone=Asia%2FTokyo&forecast_days=8&models=jma_seamless`;
...
async function showPreview(city) {
  ...
  try {
    ...
    const level = getLevelAtHour(pressures, h, hourly.relative_humidity_2m, hourly.precipitation, hourly.temperature_2m);
    ...
    buildPreviewGraph(previewData, h);
...
function updateDangerHoursLabel(startHour) {
  ...
  const RANK  = { '😊': 0, '😐': 1, '😟': 2, '😵': 3 };
  const LABEL = { '😐': '注意', '😟': '警戒', '😵': '危険' };
  const COLOR = { '😐': 'var(--warn)', '😟': 'var(--alert)', '😵': 'var(--danger)' };

  const periods = [];
  let start = -1, worst = '😊';
  for (let i = 0; i <= hourlyEmojis.length; i++) {
    const e = i < hourlyEmojis.length ? hourlyEmojis[i] : '😊';
    if (e !== '😊' && start < 0) { start = i; worst = e; }
    else if (e !== '😊' && RANK[e] > RANK[worst]) { worst = e; }
    else if (e === '😊' && start >= 0) { periods.push({ start, end: i - 1, worst }); start = -1; worst = '😊'; }
  }
  if (periods.length === 0) { el.textContent = ''; return; }

  const html = periods.map(p => {
    const sh = (startHour + p.start) % 24;
    const ehAbs = startHour + p.end + 1;
    const ehPrefix = ehAbs >= 24 ? '翌' : '';
    return `<span style="color:${COLOR[p.worst]}">${sh}〜${ehPrefix}${ehAbs % 24}時（${LABEL[p.worst]}）</span>`;
  }).join('　');
  el.innerHTML = `<div class="danger-hours-text">⚠ ${html}</div>`;
}
...
// ── グラフ描画 ──
function buildGraph(data, startHour) {
  ...
  hourlyEmojis = hourlyPressure.map((_, i) =>
    EMOJI_MAP[getLevelAtHour(pressures, startHour + i, humidity, precip, temps)]);
    ...
    options: {
      ...
      scales: {
        ...
        y: {
          display: true,
          position: 'left',
          min: Math.min(...hourlyPressure) - 2,
          max: Math.max(...hourlyPressure) + 2,
          ticks: {
            maxTicksLimit: 3,
            callback: val => Math.round(val),
            font: { size: 9, family: 'SFMono-Regular, Menlo, monospace' },
            color: 'rgba(153,153,187,0.75)',
            padding: 4,
          },
          grid: { display: false },
    ...
    plugins: [
      ...
      },
      {
        ...
        afterDraw(ch) {
          ...
          const pl = `${left}px`, pr = `${ch.width - right}px`;
          document.getElementById('graph-emoji-row').style.paddingLeft = pl;
          document.getElementById('graph-emoji-row').style.paddingRight = pr;
          document.getElementById('graph-times').style.paddingLeft = pl;
          document.getElementById('graph-times').style.paddingRight = pr;
        }
      }
    ],
  });
...
function buildDangerSummary() {
  ...
  for (let i = 0; i <= hourlyEmojis.length; i++) {
    ...
    if (e !== '😊' && start < 0) { start = i; worst = e; }
      ...
      const startH = (graphStartHour + start) % 24;
      const endAbs = graphStartHour + i;
      const endPrefix = endAbs >= 24 ? '翌' : '';
      periods.push(`${LABEL[worst]}${startH}〜${endPrefix}${endAbs % 24}時`);
      start = -1; worst = '😊';
    }
  }
  return periods.length ? periods.join('、') : 'なし';
}

async function getDailyAdvice(level, cur, diff3, diff6, humidity, weatherCode, temp, dangerSummary) {
  ...
  const diff6Str = diff6 > 0 ? `+${diff6}hPa上昇` : diff6 < -5 ? `${diff6}hPa急落` : `${diff6}hPa低下`;
...
・気圧: ${Math.round(cur)} hPa
・3時間変化: ${diffStr}
・6時間変化: ${diff6Str}
・湿度: ${humidity}%
・頭痛リスク（現在）: ${riskLabel}
・今後24時間の気圧注意時間帯: ${dangerSummary}
...
【出力ルール】
・1行目: 天気と気温のひとこと
・2行目: 気圧と頭痛リスク（注意時間帯があれば具体的に時間を記載）
...
// ── プレビュー用グラフ描画 ──
function buildPreviewGraph(data, startHour) {
  const pressures = data.hourly.surface_pressure;
  const humidity  = data.hourly.relative_humidity_2m || [];
  const precip    = data.hourly.precipitation        || [];
  const temps     = data.hourly.temperature_2m       || [];
  const sliced = pressures.slice(startHour, startHour + 25);
  const emojis = sliced.map((_, i) => EMOJI_MAP[getLevelAtHour(pressures, startHour + i, humidity, precip, temps)]);
    ...
    options: {
      ...
      scales: {
        ...
        y: {
          display: true,
          position: 'left',
          min: Math.min(...sliced) - 2,
          max: Math.max(...sliced) + 2,
          ticks: {
            maxTicksLimit: 3,
            callback: val => Math.round(val),
            font: { size: 9, family: 'SFMono-Regular, Menlo, monospace' },
    ...
    plugins: [
      ...
      },
      {
        ...
        afterDraw(ch) {
          const { left, right } = ch.chartArea;
          const pl = `${left}px`, pr = `${ch.width - right}px`;
          document.getElementById('preview-graph-emoji-row').style.paddingLeft = pl;
          document.getElementById('preview-graph-emoji-row').style.paddingRight = pr;
          document.getElementById('preview-graph-times').style.paddingLeft = pl;
          document.getElementById('preview-graph-times').style.paddingRight = pr;
        }
      }
    ],
  });
  ...
  dangerEl.innerHTML = periods.length === 0 ? '' :
    `<div class="danger-hours-text">⚠ 気圧注意 ${periods.map(([s, e]) => {
      const ehAbs = startHour + e + 1;
      const ehPrefix = ehAbs >= 24 ? '翌' : '';
      return `${(startHour + s) % 24}〜${ehPrefix}${ehAbs % 24}時`;
    }).join('　')}</div>`;
...
// ── データ反映 ──
function applyPressureData(data, currentHour, skipAI = false) {
  const h = data.hourly;
  const pressures = h.surface_pressure;
  buildGraph(data, currentHour);
  ...
  const level = getLevelAtHour(pressures, currentHour, h.relative_humidity_2m, h.precipitation, h.temperature_2m);
  const futureLevel = getLevelAtHour(pressures, Math.min(currentHour + 3, pressures.length - 1), h.relative_humidity_2m, h.precipitation, h.temperature_2m);
  ...
  document.getElementById('pressure-trend').textContent =
    trend < -3 ? 'このまま下がり続ける予測です。頭痛に注意を。' :
    trend < -1 ? 'ゆるやかに低下する予測です' :
    trend > 4  ? '気圧が急上昇しています。頭痛にご注意ください。' :
    trend > 1  ? '気圧は上昇傾向にあります' :
    'しばらく同じ水準で推移する見込みです';

  const diff6 = Math.round((cur - pressures[Math.max(0, currentHour - 6)]) * 10) / 10;
  const humidity = Math.round(h.relative_humidity_2m[currentHour]);
  const dangerSummary = buildDangerSummary();
  if (!skipAI) getDailyAdvice(displayLevel, cur, diff3, diff6, humidity, data.hourly.weather_code[currentHour], data.hourly.temperature_2m[currentHour], dangerSummary);
...
function navSettings() {
  ...
  if (el && lastFetchTime) {
    ...
    el.textContent = `最終取得: ${t.getHours()}:${String(t.getMinutes()).padStart(2, '0')}`;
  }
}
...
function checkSuddenAlert(pressures, h) {
  ...
  const direction = diff3 < 0 ? '急落' : '急上昇';
  new Notification('気圧ケア　急変アラート ⚠', {
    body: `気圧が3時間で${diff3 > 0 ? '+' : ''}${diff3}hPa${direction}しています（${Math.round(cur)} hPa）\n頭痛にご注意ください。`,
    icon: './icon-192.png',
    badge: './icon-192.png',
  });
}
...
// ── バックグラウンド復帰時の再取得 ──
const REFRESH_INTERVAL_MS = 30 * 60 * 1000; // 30分
...
async function refreshData() {
  ...
  cachedData['c'] = cData;
  const todayStr = new Date(Date.now() + 9 * 60 * 60 * 1000).toISOString().slice(0, 10);
  const sameDay = localStorage.getItem('aiAdviceDate') === todayStr;
  applyPressureData(kData, currentHour, sameDay);
...
document.addEventListener('visibilitychange', () => {
  if (!document.hidden && Date.now() - lastFetchTime > REFRESH_INTERVAL_MS) {
    refreshData().catch(e => console.error('再取得失敗:', e));
  }
});
...
async function init() {
  ...
  const notifSel = document.getElementById('notif-time');
  notifSel.value = localStorage.getItem('notifTime') || '08:00';
...
// 通知時刻セレクトの選択肢を同期的に生成（init()に依存しない）
{
  const notifSel = document.getElementById('notif-time');
  for (let h = 0; h < 24; h++) {
