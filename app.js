...
function applyPressureData(data, currentHour) {
  ...
  getDailyAdvice(level, cur, diff3, data.hourly.weather_code[currentHour], data.hourly.temperature_2m[currentHour]);
...
function scheduleNotification() {
  ...
  const timeStr = localStorage.getItem('notifTime') || '08:00';
...
async function sendMorningNotification() {
  const notifyDefault = localStorage.getItem('notifLocDefault') !== '0';
  const notifyReg = localStorage.getItem('notifLocReg') === '1';
  const h = new Date().getHours();
  const lines = [];

  try {
    if (notifyDefault) {
      const data = await fetchWeather(LOCATIONS.kagurazaka.lat, LOCATIONS.kagurazaka.lon);
      const p = data.hourly.surface_pressure;
      const cur = p[h], prev3 = p[Math.max(0, h - 3)];
      const diff3 = Math.round((cur - prev3) * 10) / 10;
      const level = getLevel(cur, diff3);
      lines.push(`神楽坂 ${Math.round(cur)} hPa ${EMOJI_MAP[level]} ${getLevelLabel(level)}`);
    }
    if (notifyReg && registeredCity) {
      const data = await fetchWeather(registeredCity.lat, registeredCity.lon);
      const p = data.hourly.surface_pressure;
      const cur = p[h], prev3 = p[Math.max(0, h - 3)];
      const diff3 = Math.round((cur - prev3) * 10) / 10;
      const level = getLevel(cur, diff3);
      lines.push(`${registeredCity.name} ${Math.round(cur)} hPa ${EMOJI_MAP[level]} ${getLevelLabel(level)}`);
    }
  } catch (_) {}

  if (lines.length === 0) return;
  new Notification('気圧ケア　今朝の気圧レポート', {
    body: lines.join('\n'),
    icon: './icon-192.png',
    badge: './icon-192.png',
  });
