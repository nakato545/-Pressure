...
function calcRiskScore(hpa, diff3h, diff6h, diff24h, humidity, precip, tempDrop6h) {
  let s = 0;
  const a6  = Math.abs(diff6h);
  const a3  = Math.abs(diff3h);
  const a24 = Math.abs(diff24h);

  // ── 正規化変化率 (hPa/h) — 3h・6hの最大値で「今の速さ」を評価 ──
  // 研究根拠: 同じ変化量でも速いほど危険。正規化で公平比較
  // 0.21 hPa/h = 5 hPa/24h（重症者閾値）, 0.33 = 8 hPa/24h（片頭痛発症ライン）
  const rateNow = Math.max(a3 / 3, a6 / 6);
...
function getLevel(hpa, diff3h, diff6h = 0, diff24h = 0) {
  return scoreToLevel(calcRiskScore(hpa, diff3h, diff6h, diff24h, 0, 0, 0));
}
...
function getLevelAtHour(pressures, h, humidity = [], precip = [], temps = []) {
  ...
  const fwdRate = Math.max(Math.abs(fwd3) / 3, Math.abs(fwd6) / 6, Math.abs(fwd12) / 12);
